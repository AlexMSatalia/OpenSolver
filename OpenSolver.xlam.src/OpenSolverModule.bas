...
Function ConvertFromCurrentLocale(ByVal s As String) As String
          ...
          'Joshua Dawes Edit 27/11/2014
          'Turns out people actually use Cell A1 of sheet 1
          'The current implementation causes a bug when the cell is merged as a title for the sheet
          'The fix i implemented was to create a worksheet when the solve button is clicked, and use a cell in this for this function.
...
301       TempWorkbook.Sheets(1).Range("A1").FormulaLocal = s
          On Error GoTo DecimalFixer
          

302       s = TempWorkbook.Sheets(1).Range("A1").Formula
303       If equalsAdded Then
304           If left(s, 1) = "=" Then s = Mid(s, 2)
305       End If
306       ConvertFromCurrentLocale = s
307       TempWorkbook.Sheets(1).Range("A1").Clear
...
311       TempWorkbook.Sheets(1).Range("A1").Clear
          TempWorkbook.Saved = True 'So that the dev isn't prompted to save after using break, which stops the workbook from being deleted.
...
DecimalFixer:
'This code catches the error caused by having the incorrect decimal character, swaps it, and continues.
' This issue arrises when the ".FormulaLocal" property expects the formula to be in the region specific formal (with region decimal character)

'This fix is very ugly, but more elegant methods (such as comparing Application.DecimalSeparator and Applicational.International(xlDecimalSeperator) ) fail
'because both of them refer to the current dominant seperator, instead of the latter refering to the one that the region would normally use.
...
325       TempWorkbook.Range("A2").Formula = s
326       s = TempWorkbook.Range("A2").FormulaLocal
327       If equalsAdded Then
328           If left(s, 1) = "=" Then s = Mid(s, 2)
329       End If
330       ConvertToCurrentLocale = s
331       TempWorkbook.Range("A2").Clear
332       Application.Calculation = oldCalculation
333       Exit Function
errorHandler:
334       TempWorkbook.Range("A2").Clear
